...
public class Adhesion {
    ...
    /*
     * 知识点: 1弧度=180/π度 Math.toDegrees(double) 转换以弧度为单位测得的角度大致相等的角度，以度衡量。 弧度->角度
     * Math.toRadians(double) 转换为度大致相等的角度，以弧度为单位的角度。 角度->弧度
     *
     * 注意：Math.sin(double)、Math.cos(double)、Math.tan(double)、Math.a_tan(double)等方法的参数
     * 均为弧度，所以如果传值来的数为角度，应该先转化为弧度
     *
     * 由于偏移角度为45，因为无论两个圆之间的角度（degree）如何变化 (x1,y1)与(x3,y3)两个点之间的角度差永远为45*2（90）
     *
     */
     ...
     * @return path 路线
     */
    public static Path drawAdhesionBody(float cx1, float cy1, float r1, float offset1, float cx2,
                                        float cy2, float r2, float offset2) {
        /* 求三角函数 */
