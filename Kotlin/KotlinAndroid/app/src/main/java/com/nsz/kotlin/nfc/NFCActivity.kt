...
class NFCActivity : AppCompatActivity(), CoroutineScope by MainScope() {
    ...
    private fun processMiFareClassic(tag: Tag) {
        launch(Dispatchers.IO) {
            try {
                ...
                miFare.connect()
                val type = miFare.type                  // 获取TAG的类型
                val blockCount = miFare.blockCount      // 获取TAG中包含的块数
                val sectorCount = miFare.sectorCount    // 获取TAG中包含的扇区数
                ...
                for (index in 0 until sectorCount) {
                    val authenticateA = miFare.authenticateSectorWithKeyA(index, MifareClassic.KEY_DEFAULT)
                    if (authenticateA) {
                        CommonLog.e("MiFareClassic 扇区$index 验证成功")
                        val blockCountInSector = miFare.getBlockCountInSector(index)
                        var sectorToBlock = miFare.sectorToBlock(index)
                        ...
                        for (pos in 0 until blockCountInSector) {
                            val readBlock = miFare.readBlock(sectorToBlock)
